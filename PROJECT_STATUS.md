...
| **Input coverage gate** | **live** | Computed from the VCF before PharmCAT runs; per-gene coverage in `quality_metrics.position_coverage` on every response. On 1000G slices it moves 1 685 results from confident to honest `Unknown` and **zero** the other way — 82.79% → 12.58%. Thresholds are probably too strict for monomorphic-filtered panels (the sweep dropped positions at random); see `docs/input_requirements.md` |
