...
| **Input requirements** | **all 7 genes require every decision-critical position** | On top of the per-gene percentage, which CYP2C19, CYP2C9 and SLCO1B1 set at 100%. Below the bar the pipeline answers confidently and *wrongly* (CYP2C9 17.4% wrong at 80% coverage), always replacing reduced function with normal. Decision-critical counts: CYP2C19 31/35, CYP2C9 69/88, CYP2D6 140/157, DPYD 28/83, NUDT15 20/20, SLCO1B1 26/35, TPMT 45/45. `docs/input_requirements.md` |
...
| **SAS breakout** | **n = 601** | CYP2C19 reduced-function (IM+PM) **57.1%** (343/601), 95% CI [53.1, 61.0]. Every subcohort now n≥103 (BEB 131, PJL 146, STU 114, ITU 107, GIH 103), so each supports a rate — but they are **not distinguishable from each other**, GIH 49.5% and STU 64.0% still overlapping. The earlier 53.3% at n=75 sits inside the new interval: it was imprecise, not wrong. Four of five cohorts were collected outside India; the coverage gate declines all 601, so these are PharmCAT's raw calls, not product output. `reports/sas_cohort_expansion.md`. Allele-frequency deviations against CPIC were filtered for coverage and nomenclature artifacts: of 30 rows, **6 are input artifacts** (all reference alleles — `*1` needs every position, and NUDT15 `*1` was called from 5 of 20), **9 have no CPIC row**, 9 agree, and 6 differ — none attributable to population genetics once CPIC's broader Central/South Asian grouping is accounted for. `reports/sas_deviation_filtered.md` |
...
| Item | State | Blocker |
| --- | --- | --- |
| **Human adjudication of explanation prose** | **Provisional** — 124 decided by a person, **36 by automated alignment**, 19 escalated and outstanding | Human judgement; the release gate stays red until it is done. The explanation store is frozen while this runs |
| **Phase 5B — warfarin dose regressor + SHAP (trimmed to this alone)** | Not started | CYP2D6 needs a trustworthy external caller (Stargazer/Cyrius or a lab report); `-po` is already supported in 3.4.0 and research mode stays disabled deliberately. **"Indian population priors" DROPPED as redundant** — — the Phase 6 SAS result (n=75, CYP2C19 reduced-function 53.3%, allele frequencies agreeing with CPIC's Central/South Asian figures) already delivers what it was scoped to provide, from measured data rather than priors. Recorded rather than silently deleted: the item asked for population-specific frequency priors for South Asian patients; Phase 6 measured them directly against CPIC's published Central/South Asian figures, which is strictly better evidence than a prior. **CYP2D6 external diplotype and the encrypted offline cache moved to future work**, with the reasoning recorded: this project's contribution is the verification findings — eight defects in its own honesty layer, all sharing one structural direction, and the five-edge verification graph that found them. Additional features dilute that rather than strengthen it, and each would add surface needing its own validation before its output could be trusted. The warfarin regressor is retained because it is the ML component the project is nominally about |
...
| **SLCO1B1 phenotype-without-diplotype state** | Built (Phase 6 final) | Concordant candidates now produce a confident label |
| **Allele-scoped coverage requirement** | **Partly done** | Every gene now also requires each position defining a non-normal-function allele, derived from PharmCAT's `functionValue` rather than hand-picked. This **adds** a condition; it does not yet **replace** the percentage thresholds, which is the remaining half. DPYD formerly cleared 37.3% while CYP2C9 failed at 19.3% — identity, not count |
...
| item | why it is open |
| --- | --- |
| ~~**Constructed-allele systematic sweep** (audit A §5)~~ **DONE 2026-08-13** | All **503** named alleles built at complete coverage and run end to end. **Zero** failed to call, called wrongly, or produced an unsupported label. 320/331 callable-gene alleles exact; 2 SLCO1B1 sub-alleles correctly collapsed to the core `*45` (the phenotype table has no sub-allele entry); 9 NUDT15 correctly deferred to TPMT; CYP2D6 declined 172/172 by design. Coverage testing, **not** external validation — external concordance remains n=1. `reports/constructed_allele_sweep.md` |
| ~~**Client-state coverage** (audit A §7)~~ **DONE 2026-08-13** | `app/test/state_census_test.dart` renders every state at **360px**: all 5 risk labels, all 5 Unknown reasons, all 4 backend phases, the version-skew notice, and all six captured S1–S6 payloads. States are enumerated **from their enums**, not from a list, so a new value is exercised without anyone remembering to add it. Contract parity (Pydantic↔Dart, field by field) remains covered by `contract_test.dart`. |
